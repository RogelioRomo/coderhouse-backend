class ProductManager{
    ...
    addProduct(product) {
        if(!product.title || !product.description || !product.price || !product.thumbnail || !product.code || !product.stock) return 'Missing product details'
        

        const productCode = this.products.find(prod => prod.code === product.code)
        
        if (productCode) return 'This code already exists in other product, please change'
